...
export function Toolbar({ title, buttons, className = '' }: ToolbarProps) {
  return (
    <div className={`bg-gradient-to-r from-slate-50 to-gray-100 border-b border-slate-200 px-3 py-2 shadow-sm ${className}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1">
          {buttons.map((button) => {
            const Icon = button.icon
            const variantClasses = button.variant === 'primary' 
              ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white hover:from-blue-700 hover:to-blue-800 shadow-md hover:shadow-lg border-blue-600 transform hover:scale-105'
              : button.variant === 'danger'
              ? 'bg-gradient-to-r from-red-500 to-red-600 text-white hover:from-red-600 hover:to-red-700 shadow-md hover:shadow-lg border-red-500 transform hover:scale-105'
              : 'bg-gradient-to-r from-white to-gray-50 text-gray-700 hover:from-gray-50 hover:to-gray-100 border-gray-300 shadow-sm hover:shadow-md transform hover:scale-105'
