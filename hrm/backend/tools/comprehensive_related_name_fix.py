#!/usr/bin/env python3
"""
Comprehensive script to fix all related_name conflicts in HRM models.
Based on the analysis of 1,234 Django system check errors.

This script will:
1. Parse all model files in hrm/backend/hrm/models/
2. Identify ForeignKey fields without unique related_name arguments
...
class RelatedNameFixer:
    ...
    def fix_all_conflicts(self, apply_changes=False):
        ...
        print("🔧 Starting comprehensive related_name conflict fix...")
        ...
        print(f"🏗️  Found {len(all_models_info)} models with {sum(len(m['fields']) for m in all_models_info)} fields")
        
        # Identify conflicts
        conflicts = self.identify_conflicts(all_models_info)
        print(f"⚠️  Identified {len(conflicts)} related_name conflicts")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Fix related_name conflicts in HRM models')
    ...
    fixer = RelatedNameFixer(args.models_dir)
